...
export default function GenreSelectorHate() {
          ...
          <div className="flex-row flex-wrap gap-8">
            {GENRES.map((x) => {
              return (
                <button
                  name={x.genreId}
                  className={`font-semibold h-12 w-28 my-2 mx-1 ${genreCtx.loveIt.includes(x.genreId) ? "bg-green-300" : genreCtx.hateIt.includes(x.genreId) ? "bg-red-300" : "bg-gray-300"} rounded transition-all duration-200  ${genreCtx.loveIt.includes(x.genreId) ? "hover:bg-green-400" : genreCtx.hateIt.includes(x.genreId) ? "hover:bg-red-400" : "hover:bg-gray-400"}`}
